...
function updateTotalCals() {
  //watch each one of the total calorie table slots
    $(".meal-cals-total").each(function() {
      $(this).on("DOMSubtreeModified", function() {
        //when there's a change, grab all the values and add them together.
        var value = 0
        $(".meal-cals-total").each(function() {
          value += parseInt(this.textContent)
        })
        //append value to calories consumed.
        $("#calories-consumed").text(value)
        //compare with the goal calories. Can hardcode this.
